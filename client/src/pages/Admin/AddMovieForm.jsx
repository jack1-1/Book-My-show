...
import { addMovie } from '../../calls/movieCalls';
import { updateMovie } from '../../calls/movieCalls';
function AddMovieForm({ isModalOpen, setIsModalOpen, formType, selectedMovie,setSelectedMovie}) {
  const onFinish = async (values) => {
    if (formType == "add") {
      ...
      try {
        const response = await updateMovie({...values,'movieId':selectedMovie._id});
        if (response.success) {
          message.success(response.message);
  ...
  return (
    <Modal open={isModalOpen} width={800} onCancel={handleCloseModal}>
